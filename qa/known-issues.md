...
| P0-006 | 2026-07-02 | 🔴 Bloquant | Backend Viim déployé mais health check en `degraded` car `NEWAGENT_TOKEN` n'est pas configuré dans Coolify. `NEWAGENT_URL` est présent côté runtime et l'endpoint candidat `https://burktech-ia.com/health` répond HTTP 200 ; le token réel reste à fournir par Guy. | Backend / NEwAGENT-IA | Ajouter le vrai `NEWAGENT_TOKEN` dans Coolify, puis redéployer ou redémarrer l'app Viim. Ne pas utiliser de faux token. |
