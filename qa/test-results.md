...
|---|---|---|---|---|---|---|
| 2026-07-04 | ASSIST-005/006 actions Assistance | iPhone de Guy + iPhone 17 Simulator | N/A | OK build installé | `xcodebuild test` OK ; nouveau test `testForegroundLocationRequestDoesNotPromotePassiveWakeup` OK ; build signé iPhone réel OK ; installation OK ; lancement `com.yamstack.viim` OK ; `/health` public OK | Le bouton test n'est plus grisé sans explication : il ouvre la configuration contact si nécessaire. `Voir ma localisation` déclenche une demande GPS ponctuelle. Appui réel dans l'UI à retester par Guy sur l'iPhone. |
| 2026-07-04 | ASSIST-004 format contact | iPhone de Guy + iPhone 17 Simulator | N/A | OK correctif | `xcodebuild test` OK ; `BurkinaPhoneNumberTests` OK ; build signé iPhone réel OK ; installation OK ; lancement `com.yamstack.viim` OK | Cause reproduite sans envoi réel : l'API rejette `+226 70 00 00 00` en HTTP 422. L'app normalise désormais ce format en `+22670000000` avant stockage/envoi. Test WhatsApp réel encore à lancer avec un numéro consenti. |
